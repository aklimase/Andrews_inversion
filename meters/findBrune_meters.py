...
event_spectra_dir = boxpath + '/secondo_3plus/'
...
print 'Number of events mag ', mag_lb, ' - ', mag_ub, ': ', len(event)
...
#    plt.loglog(freq , Brune_demean_list[i],color = 'blue', ls = '--', label = 'demeaned Brune spectra')
    ...
    plt.text(1, .02, 'Avg log(diff) 0.1-36Hz: ' + str(round(np.mean(cf_list[i][21:48]),3)), fontsize = 16)
...
sum_list = map(sum,cfarray[:,np.arange(21,48)]**2.0) ###found the best fit from 0.5-36Hz
print sum_list
...
outfile = open(boxpath + '/constraint_' + ev_list[ind] + '.out', 'w')
