...
hardstart = boxpath + '/record_spectra_rebin3/Event_2010_05_25_19_49_51/AZ_BZN_HHNE__2010_05_25_19_49_51.out'
...
#    plt.xlabel('counts')
    plt.annotate('counts', va='center',  xy = (0.48,-0.1), xycoords = 'axes fraction')
    plt.annotate('(a)', xy = (0.0,1.1), xycoords = 'axes fraction')
    
    plt.subplot(gs.new_subplotspec((1, 0), colspan=1))
    plt.xlim(0.1,70)
    plt.grid()
    plt.loglog(f_bins, record_spec, label = 'record', c= 'cornflowerblue')
    plt.tick_params(axis='both', which='major')
    plt.tick_params(axis='both', which='both', length = 5, width = 1)
    plt.legend(loc=3, borderaxespad=0.)
    plt.annotate('(b)', xy = (0.0,1.02), xycoords = 'axes fraction')
    ...
    plt.xlim(0.1,70)
    plt.grid()
    plt.loglog(f_bins, record_spec*dist_m, label = 'record dist corr', c= 'cornflowerblue')
    plt.loglog(f_bins, station_spec*event_spec, color='black', label = 'event*site')
    plt.legend(loc=3, borderaxespad=0.)
    plt.tick_params(axis='both', which='major')
    plt.tick_params(axis='both', which='both', length = 5, width = 1)
    plt.annotate('(c)', xy = (0.0,1.02), xycoords = 'axes fraction')
    ...
    plt.grid()
    plt.loglog(f_bins, event_spec, color = 'green', label = 'event ' + eventid)
    plt.loglog(f_bins, Brune, ls = '--', color = 'green', label = 'Brune source')
    plt.legend(loc=3, borderaxespad=0.)
    plt.tick_params(axis='both', which='major')
    plt.tick_params(axis='both', which='both', length = 5, width = 1)
    plt.annotate('(d)', xy = (0.0,1.02), xycoords = 'axes fraction')
    ...
    plt.xlim(0.1,70)
    plt.grid()
    plt.loglog(f_bins, station_spec, color='r', label = 'station ' + station)
    plt.legend(loc=3, borderaxespad=0.)
    plt.tick_params(axis='both', which='major')
    plt.tick_params(axis='both', which='both', length = 5, width = 1)
    plt.xlabel('Frequency (Hz)')
    plt.annotate('(e)', xy = (0.0,1.02), xycoords = 'axes fraction')
