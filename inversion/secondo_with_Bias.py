...
#record_path =[]
#for ev in ev_list:
#    record_path.extend(glob.glob(boxpath + '/record_spectra/Event_' + ev + '/*.out'))
#    record_path.extend(glob.glob(boxpath + '/record_spectra/Event_' + ev + '/*.out'))
...
outfile_path = boxpath + '/secondo_Bias'
...
#for C
#make each row into an array
amp = np.sqrt(np.power(10.0, C[0,:]))
####
outfile = open(outfile_path + '/' + 'Bias.out', 'w')
out = (np.array([freq_list, amp])).T
outfile.write('#freq_bins \t vel_spec_NE_cm \n')
np.savetxt(outfile, out, fmt=['%E', '%E'], delimiter='\t')
outfile.close()
#
##plot C
#data = np.genfromtxt(outfile_path + '/' + 'C_bias.out', dtype = float, comments = '#', delimiter = None, usecols = (0,1), encoding = None)#only read in first two cols
#freq = data[:,0][30:48]
#print freq
...
#plt.loglog(freq, C)
#
#decay = C0*np.exp(-np.pi*(freq)/(Q*Beta))
#plt.loglog(freq, decay)
#
#plt.grid()
